...
HOW TO RUN:
  1. First run the Jupyter Notebook (AquaVision_Notebook.ipynb) to generate
     the 'aquavision_assets/' folder with saved models and clean data.
  2. Place this file in the SAME directory as aquavision_assets/
  3. Install: pip install streamlit plotly scikit-learn xgboost lightgbm shap joblib pandas
  4. Run:     streamlit run aquavision_app.py
"""
...
<style>
    ...
    .section-title {
        ...
        color: #0D47A1; margin: 1.5rem 0 1rem;
        padding-left: 0.8rem;
    ...
    div[data-testid="stSidebar"] .stSelectbox label { color: rgba(255,255,255,0.8) !important; }
    div[data-testid="stSidebar"] .stSlider label   { color: rgba(255,255,255,0.8) !important; }
</style>
""", unsafe_allow_html=True)


# ── Helper Functions ───────────────────────────────────────────────────────────
ASSET_DIR = 'aquavision_assets'

@st.cache_data
def load_data():
    path = os.path.join(ASSET_DIR, 'clean_data.csv')
    if os.path.exists(path):
        return pd.read_csv(path)
    # Fallback: regenerate from raw
...
@st.cache_resource
def load_models():
    reg  = joblib.load(os.path.join(ASSET_DIR, 'wqi_regressor.pkl'))
    clf  = joblib.load(os.path.join(ASSET_DIR, 'wqi_classifier.pkl'))
    sc   = joblib.load(os.path.join(ASSET_DIR, 'scaler.pkl'))
    le   = joblib.load(os.path.join(ASSET_DIR, 'label_encoder.pkl'))
    feat = joblib.load(os.path.join(ASSET_DIR, 'feature_names.pkl'))
    return reg, clf, sc, le, feat
...
def engineer_features(row_dict, feature_names):
    d = row_dict.copy()
    ...
    return [d[f] for f in feature_names]
...
models_loaded = False
try:
    reg_model, clf_model, scaler, label_enc, FEATURES = load_models()
    models_loaded = True
except Exception as e:
    st.warning(f"⚠️ Models not found. Run the Jupyter notebook first to generate models. ({e})")
...
if page == "🏠 Dashboard":
    ...
    with col1:
        ...
        cat_colors_map = {'Excellent':'#1B5E20','Good':'#4CAF50','Poor':'#FF9800',
                          'Very Poor':'#F44336','Unfit for Drinking':'#880E4F'}
        fig = px.pie(cat_cnt, names='Category', values='Count',
                     color='Category', color_discrete_map=cat_colors_map,
                     title='Distribution of WQI Categories',
                     hole=0.42)
    ...
    with col2:
        ...
        fig.add_trace(go.Scatter(
            x=yr_wqi['year'], y=yr_wqi['mean'] + yr_wqi['std'],
        ...
        fig.add_trace(go.Scatter(
            x=yr_wqi['year'], y=yr_wqi['mean'], mode='lines+markers',
            line=dict(color='#1565C0', width=3),
            marker=dict(size=7), name='Mean WQI'))
        fig.update_layout(title='Year-wise WQI Trend',
                          xaxis_title='Year', yaxis_title='WQI Score',
                          template='plotly_white', height=380)
        st.plotly_chart(fig, use_container_width=True)

    st.markdown('<div class="section-title">Parameter Overview</div>', unsafe_allow_html=True)
    num_cols_list = ['temp','do','ph','conductivity','bod','nitrate']
    cols = st.columns(6)
    for i, col_name in enumerate(num_cols_list):
        val = dff[col_name].mean()
        cols[i].metric(col_name.upper(), f"{val:.2f}")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        fig.add_trace(go.Histogram(x=dff[selected_param], nbinsx=50,
                                   ...
                                   name=selected_param), row=1, col=1)
        for cat, color in [('Excellent','#1B5E20'),('Good','#4CAF50'),('Poor','#FF9800'),
                            ('Very Poor','#F44336'),('Unfit for Drinking','#880E4F')]:
    ...
    with tab3:
        fig = make_subplots(rows=2, cols=4,
                            subplot_titles=[c.upper() for c in num_cols_list])
        cat_order = ['Excellent','Good','Poor','Very Poor','Unfit for Drinking']
        cat_col_map = {'Excellent':'#1B5E20','Good':'#4CAF50','Poor':'#FF9800',
                       'Very Poor':'#F44336','Unfit for Drinking':'#880E4F'}
        for idx, col_name in enumerate(num_cols_list):
            ...
            for cat in cat_order:
                ...
                if len(sub) > 0:
                    fig.add_trace(
                        go.Box(y=sub, name=cat, marker_color=cat_col_map[cat],
                               showlegend=(idx == 0)),
                        row=r+1, col=c+1)
    ...
    with tab4:
        ...
        sc = StandardScaler()
        X_pca = sc.fit_transform(dff[num_cols_list].fillna(dff[num_cols_list].mean()))
        pca = PCA(n_components=3)
        coords = pca.fit_transform(X_pca)
        pca_df = pd.DataFrame({
            'PC1': coords[:,0], 'PC2': coords[:,1], 'PC3': coords[:,2],
            'Category': dff['WQI_Category'].values,
            'WQI': dff['WQI'].values
        })
        col_pca1, col_pca2 = st.columns([2,1])
        with col_pca1:
            fig = px.scatter_3d(pca_df, x='PC1', y='PC2', z='PC3',
                                color='Category', color_discrete_map=cat_col_map,
                                opacity=0.65, size_max=6,
                                title=f'PCA 3D — Explains {sum(pca.explained_variance_ratio_)*100:.1f}% Variance',
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        fig = go.Figure()
        colors_trend = px.colors.qualitative.Bold
        for i, param in enumerate(params_sel):
            fig.add_trace(go.Scatter(
                x=yr_agg['year'], y=yr_agg[param],
        ...
        fig2 = px.area(cat_yr, x='year', y='count', color='WQI_Category',
                       color_discrete_map={
                           ...
                           'Very Poor':'#F44336','Unfit for Drinking':'#880E4F'},
                       title='WQI Category Composition Over Years')
    ...
    with tab2:
        ...
        fig = px.bar(state_param.reset_index(), x='state', y=param_dd,
                     ...
                     color=param_dd, color_continuous_scale='Blues',
                     text_auto='.2f')
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not models_loaded:
        st.error("⚠️ Models not loaded. Please run the Jupyter notebook first to train and save models.")
        st.stop()
    ...
    if st.button("🚀 Predict Water Quality Index", use_container_width=True, type="primary"):
        ...
        feat_vec = engineer_features(row, FEATURES)
        ...
        with col_r3:
            ...
            probs = pd.DataFrame({
                'Category': label_enc.classes_,
                'Probability': cat_prob * 100
            }).sort_values('Probability', ascending=True)
            fig = px.bar(probs, x='Probability', y='Category',
                         orientation='h', title='Category Confidence (%)',
        ...
        fig = px.bar(compare, x='Parameter', y='Value', color='Source',
                     barmode='group', template='plotly_white',
                     color_discrete_sequence=['#1565C0', '#80DEEA'],
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        fig = px.bar(state_stats.sort_values('avg_wqi', ascending=False),
                     ...
                     text='avg_wqi',
                     labels={'avg_wqi': 'Average WQI', 'state': 'State'})
    ...
    with col2:
        fig2 = px.treemap(state_stats, path=['state'], values='records',
                          ...
                          title='State Coverage (by records)',
                          hover_data=['avg_wqi','category'])
        fig2.update_layout(height=450)
        st.plotly_chart(fig2, use_container_width=True)

    st.markdown("**📋 State Summary Table**")
    st.dataframe(
        state_stats.sort_values('avg_wqi', ascending=False).reset_index(drop=True),
        use_container_width=True, height=350
    )

    # Radar chart for top states
    top5 = state_stats.nlargest(5, 'records')['state'].tolist()
    radar_params = ['avg_do','avg_ph','avg_bod','avg_wqi']
    fig_radar = go.Figure()
    colors_r = ['#1565C0','#00897B','#E53935','#7B1FA2','#F57C00']
    for i, st_name in enumerate(top5):
        ...
        fig_radar.add_trace(go.Scatterpolar(
            r=vals, theta=radar_params + [radar_params[0]],
            fill='toself', name=st_name,
            line=dict(color=colors_r[i]), opacity=0.6
        ))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col2:
        cat_filter = st.selectbox("Filter by WQI Category",
                                   ['All'] + list(dff['WQI_Category'].unique()))

    df_show = dff.copy()
    if search:
        df_show = df_show[
            df_show['location'].str.contains(search, case=False, na=False) |
    ...
    st.dataframe(
        df_show[['station_code','location','state','year',
                 'temp','do','ph','conductivity','bod','nitrate',
                 'WQI','WQI_Category']].reset_index(drop=True),
    ...
    st.download_button(
        ...
        file_name='aquavision_filtered.csv',
        mime='text/csv'
    )
...
<div style='text-align:center; color:#888; font-size:0.85rem; padding:1rem'>
    ...
    Final Year Project &nbsp;|&nbsp; Built with Streamlit + XGBoost + SHAP
</div>
""", unsafe_allow_html=True)
